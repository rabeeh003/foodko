        ...
        <div className="p-4 rounded shadow-lg">
            ...
            <div className="sm:flex justify-between items-center mb-4">
                <div className="flex items-center gap-1 mt-1">
                    <input type="date" className="input input-bordered max-w-xs" value={filterDate} onChange={handleDateFilterChange} />
                    <select className="input input-bordered max-w-xs" value={filterStatus} onChange={handleStatusFilterChange}>
                ...
                <button className="w-full sm:w-fit px-4 py-2 mt-1 btn btn-outline-primary" onClick={() => openEditModal()}>Add Order</button>
            </div>
                    ...
                    <tbody>
                        {currentOrders.map((order) => (
                            <tr key={order.id} className="border-b">
                                ...
                                <td className="p-3">
                                    <button className={`btn btn-sm ${order.status === "Pending" ? 'btn-solid-primary' : order.status === "Delivered" ? 'btn-solid-success' : 'btn-solid-error'}`}>
                                        {order.status}
                                    </button>
                                </td>
                                <td className="p-3 flex gap-2">
                                    ...
                                    <button className="p-2 btn btn-solid-error" onClick={() => openDeleteModal(order)}>
                                        <Trash />
                                    </button>
                                </td>
                            </tr>
            ...
            <div className="flex justify-between items-center mt-4">
                <button onClick={() => handlePageChange(currentPage - 1)} disabled={currentPage === 1} className="px-3 py-1 bg-gray-200 text-gray-500 rounded">
                    Previous
                </button>
                <span>Page {currentPage} of {totalPages}</span>
                <button onClick={() => handlePageChange(currentPage + 1)} disabled={currentPage === totalPages} className="px-3 py-1 bg-gray-200 text-gray-500 rounded">
                    Next
                </button>
            </div>
                ...
                <div className="sm:ms-[18rem] fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
                    <div className="bg-backgroundPrimary rounded-lg p-6 w-96">
                        ...
                        <div className="flex justify-end mt-4">
                            <button className="btn btn-solid-primary" onClick={closeDeleteModal}>Cancel</button>
                            <button className="btn btn-solid-error ml-2" onClick={() => deleteOrder(selectedOrder._id)}>Delete</button>
                        </div>
                    </div>
                </div>
            )}
